...
function isValidManifestTrack(track) {
  return (
    track &&
    typeof track === 'object' &&
      ...
      {queueTracks.map((track, idx) => (
        <div key={`${track.id}-${idx}`} className="queue-row">
          ...
          <div className="queue-actions">
            ...
            <button onClick={() => onMove(idx, idx + 1)} disabled={idx === queueTracks.length - 1}>
              ↓
            </button>
    ...
    <div className="deck">
          ...
          {radioPhase === 'announcing' ? (
            ...
            <>
              ...
              <span className="onair-time">
                {radioTimeLeft !== null
                  ? `Next in ${String(Math.floor(radioTimeLeft / 60)).padStart(2, '0')}:${String(radioTimeLeft % 60).padStart(2, '0')}`
                  : ''}
              </span>
            </>
          )}
      ...
      <div className="deck-aux">
        <div className="aux-row">
          ...
          <button
            className={visualMode === 'waveform' ? 'aux-btn aux-btn--on' : 'aux-btn'}
            onClick={() => onVisualMode('waveform')}
          >
            Wave
          </button>
          <button
            className={visualMode === 'spectrum' ? 'aux-btn aux-btn--on' : 'aux-btn'}
            onClick={() => onVisualMode('spectrum')}
          >
            Spectrum
          </button>
          <button className="aux-btn" onClick={onExport} disabled={!audioReady}>
            {exporting ? 'Stop Export' : 'Export WAV'}
          </button>
        </div>
      </div>
      ...
      <div className="deck-knobs deck-knobs--wide">
        ...
        <div className="deck-vol">
          ...
          <Knob
            ...
            onChange={onRjVolume}
            trackColor="#ffa800"
            glowColor="rgba(255,168,0,.6)"
            dragTitle="RJ volume"
            dragDirection={-1}
          />
          <span className="vol-val">
            {Math.round(rjVolume * 100)}
            <small>%</small>
          </span>
        </div>
      </div>
    ...
    fetch('/tunes/manifest.json')
      ...
      .then((data) => {
        if (Array.isArray(data) && data.length) {
          ...
          if (valid.length) {
            setManifestTracks(valid.map(normalizeTrack));
          }
        }
      })
      .catch(() => {});
    ...
    <div className="app">
      <header className="app-hd">
        ...
        <div className="hd-tools">
          <button className="mini-btn" onClick={() => setShowEditor((v) => !v)}>
            Editor
          </button>
          <button className="mini-btn" onClick={() => setShowShortcuts((v) => !v)}>
            Shortcuts
          </button>
          <button
            className="mini-btn"
      ...
      <TuneEditor
        open={showEditor}
        draft={draftTune}
        onDraft={setDraftTune}
        onClose={() => setShowEditor(false)}
        onSave={saveDraftTune}
        onPlay={playDraftTune}
      />
      ...
      <Deck
        ...
        onRadioToggle={() => {
          const idx = Math.max(
            0,
            loadedTrack ? tracks.findIndex((t) => t.id === loadedTrack.id) : 0,
          );
          radio.toggle(idx);
        }}
