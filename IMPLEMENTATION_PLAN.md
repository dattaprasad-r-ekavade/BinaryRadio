...
- [x] `Knob.jsx` — SVG circular potentiometer (zero shared state)
- [x] `Reel.jsx` — spinning cassette reel SVG
- [x] `Visualizer.jsx` — canvas spectrum/waveform display
- [x] `VUMeter.jsx` — animated level indicators
- [x] `Deck.jsx` — cassette player interface (transport controls, visualizer, knobs)
- [x] `CassetteCard.jsx` — individual tape in the rack
- [x] `QueuePanel.jsx` — playlist queue with reorder/remove/clear
- [x] `Rack.jsx` — 3-column grid of cassettes with search/filter
- [x] `TuneEditor.jsx` — in-browser Strudel code editor
- [x] `HelpModal.jsx` — keyboard shortcuts cheat sheet
- [x] Keep `App.jsx` as the thin orchestrator (~200 lines max)
...
- [x] Create `src/hooks/usePlayerState.js` (or `src/state/playerContext.js`)
- [x] Consolidate the 20+ `useState` calls in App.jsx into a single state hook/context
- [x] Group state: transport (play/stop/cps/loop), library (tracks/favorites), queue, UI (theme/view/modal)
- [x] Use `useReducer` for complex state transitions
...
- [x] `Knob.test.jsx` — renders, drag interaction changes value
- [x] `CassetteCard.test.jsx` — renders metadata, click triggers load
- [x] `Rack.test.jsx` — renders all tracks, search filters correctly
- [x] `QueuePanel.test.jsx` — reorder, remove, clear operations
- [x] `Deck.test.jsx` — transport button states reflect deckState
- [x] `TuneEditor.test.jsx` — save/load custom tune, validation errors shown

### B4. Integration Tests

- [x] Test: load tune file → parse → normalize → ready for playback
- [x] Test: `useStrudel` initialization → ready state → error state
- [x] Test: `useRadioMode` state machine: idle → playing → announcing → next
- [x] Test: WAV export produces valid RIFF header
...
- [x] Add PropTypes to all extracted components at minimum
- [x] Optional: consider TypeScript migration (create `tsconfig.json`, rename files incrementally)
- [x] Document the decision in CONTRIBUTING.md

### B8. CSS Modularization

- [x] Split `App.css` (663 lines) into per-component CSS files or CSS modules
- [x] Each component imports its own styles
- [x] Shared design tokens remain in a `variables.css` or `:root` block
...
- [x] Add ARIA labels to all interactive elements (knobs, buttons, transport controls)
- [x] Ensure keyboard focus management (tab order, focus rings)
- [x] Screen reader testing: announce track changes, play state
- [x] Color contrast check for both themes (WCAG AA minimum)
- [x] Knob interaction: add keyboard control (arrow keys) alongside mouse/touch
...
- [x] Test: load app ? click tape ? press play ? verify audio context created
- [x] Test: search/filter rack ? correct tapes shown
- [x] Test: keyboard shortcuts work (space, S, N, P)
- [x] Test: theme toggle persists across reload
- [x] Add E2E to CI pipeline
...
- [x] Lazy-load TuneEditor (React.lazy + Suspense) — it's heavy and rarely used
- [x] Throttle visualizer animation to 30fps on low-end devices
- [x] Profile and optimize `extractSampleSelectors()` regex for large tune files
- [x] Add `loading="lazy"` to any images if added

### C8. Demo & Deployment

- [x] Deploy to GitHub Pages or Vercel (add deploy script / GitHub Action)
- [x] Add live demo link to README
- [x] Add Open Graph meta tags for social sharing preview
- [x] Record a 30-second demo GIF for README
...
- [x] All Phase A items complete � CI green, linting enforced, community files in place
- [x] All Phase B items complete — components extracted, tests passing, error boundary active
- [x] Key Phase C items complete � accessibility, E2E tests, live demo deployed
- [x] README has screenshot/GIF, architecture diagram, badges, and accurate feature list
